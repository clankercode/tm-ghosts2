...
- [x] Load ghosts from replay files (`Replay_Load` → `RaceGhost_Add`) — needs in-game test with a real file (opus subagent)
...
- [ ] Save ghost (`Replay_Save` exists; `Ghost_Save` does not) — verify path/format in-game
...
- [~] Ghidra headless analysis of ManiaPlanet.exe (~/re/mp4, systemd unit) → GhidraMCP server :18743 (grok helper) → research/mp4/Ghidra.md
