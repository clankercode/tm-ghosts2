#!/usr/bin/env bash
# Build / stage / hot-reload an Openplanet plugin for ManiaPlanet 4 (Openplanet4) or Trackmania Turbo.
#   ./build.sh [dev|release]           (default: dev, game MP4)
#   GAME=turbo ./build.sh dev          stage into ~/OpenplanetTurbo and reload over RemoteBuild :30002
# Env: GAME (mp4|turbo, default mp4), MP4_PLUGINS_DIR (NOT PLUGINS_DIR: that env var points at the TM2020
#      tree) (default ~/Openplanet4/Plugins), TURBO_PLUGINS_DIR (default ~/OpenplanetTurbo/Plugins),
#      SKIP_LSP=1, SKIP_RELOAD=1
set -euo pipefail
cd "$(dirname "$0")"
mode="${1:-dev}"
game="${GAME:-mp4}"
...
if [[ "${SKIP_LSP:-0}" != "1" ]] && command -v openplanet-lsp >/dev/null; then
  ...
  lsp_args=(--game-target "$lsp_target" --plugins-dir "$plugins_dir" --plugins-dir "$(dirname "$PWD")")
  # MP4's install folder is named Openplanet4, which the LSP does not look for by itself; Turbo's
  # OpenplanetTurbo is found automatically, so no --typedb-dir there.
